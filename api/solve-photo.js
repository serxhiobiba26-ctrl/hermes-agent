...
// Prova prima il modello più intelligente, poi ripiega su uno sempre disponibile.
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    let j = null, ok = false, lastErr = "Errore dal servizio AI.";
    for (const model of MODELS) {
      const url = "https://generativelanguage.googleapis.com/v1beta/models/" +
        model + ":generateContent?key=" + encodeURIComponent(key);
      const apiResp = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: reqBody });
      j = await apiResp.json();
      if (apiResp.ok) { ok = true; break; }
      lastErr = (j && j.error && j.error.message) ? j.error.message : lastErr;
      // se il modello non esiste/non è disponibile, prova il prossimo; altrimenti esci subito
      if (!/not found|not supported|unknown|does not exist|invalid|unavailable|404/i.test(lastErr)) break;
    }
    if (!ok) {
